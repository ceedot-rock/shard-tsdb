...
exports.BlackjackCodec = exports.BlackjackV2 = exports.ShardTSDB = void 0;
...
function encodeTimestamps(tss) {
    ...
    for (let i = 1; i < tss.length; i++) {
        deltas.push(tss[i] - prev);
        prev = tss[i];
    }
    const dod = [tss[0], deltas[0]];
    for (let i = 1; i < deltas.length; i++)
        dod.push(deltas[i] - deltas[i - 1]);
    return dod.map(v => v >= 0 ? v * 2 : (-v) * 2 - 1);
}
function decodeTimestamps(dod) {
    if (dod.length === 0)
        return [];
    if (dod.length === 1)
        return [dod[0]];
    ...
    for (let i = 2; i < unzig.length; i++) {
        delta += unzig[i];
        tss.push(tss[tss.length - 1] + delta);
    }
    return tss;
}
function keyStr(name, tags) { return name + "|" + Object.entries(tags).sort().map(function (p) { return p[0] + "=" + p[1]; }).join(","); }
class MemoryBackend {
    constructor() {
        this.blocks = new Map();
    }
    async append(key, block) { if (!this.blocks.has(key))
        this.blocks.set(key, []); this.blocks.get(key).push(block); }
    async getBlocks(key) { return this.blocks.get(key) || []; }
}
class IndexedDBBackend {
    constructor() {
        this.dbName = 'shard-tsdb';
    }
    async append(key, block) {
    ...
    async getBlocks(key) {
        try {
            const all = JSON.parse(localStorage.getItem(this.dbName) || '{}');
            const arr = all[key] || [];
            return arr.map((b) => ({ tPacked: new Uint8Array(b.t), vPacked: new Uint8Array(b.v), count: b.count, minTs: b.minTs, maxTs: b.maxTs }));
        }
        catch {
            return [];
        }
...
class ShardTSDB {
    ...
    async flushBlock(key, tss, vals) {
        ...
        const block = { tPacked: packBits(tBits), vPacked: packBits(vBits), count: tss.length, minTs: tss[0], maxTs: tss[tss.length - 1] };
    ...
    async range(name, tags, start, end) {
        await this.flushAll();
        const k = keyStr(name, tags);
